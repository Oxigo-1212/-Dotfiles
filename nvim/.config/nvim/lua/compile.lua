...
M.last_cwd = nil
--- @type string|nil Path to the last .env file used (consumed once per execution).
M.last_env = nil
--- @type integer|nil
...
--- @return nil
M.command = function()
	local ok, cmd = pcall(vim.fn.input, "Compile command: ", M.last_cmd or "")
	if not ok or cmd == nil or cmd == "" then
		return
	end

	local default_cwd = M.last_cwd or vim.fn.getcwd()
	local ok2, cwd = pcall(vim.fn.input, "CWD: ", default_cwd, "dir")
	if not ok2 or cwd == nil or cwd == "" then
		return
	end
...
--- @return nil
M.with_env = function()
	local ok, env_file = pcall(vim.fn.input, "Path to .env file: ", M.last_env or vim.fs.joinpath(vim.fn.getcwd(), ".env"),
		"file")
	if not ok or env_file == nil or env_file == "" then
		return
	end
	M.last_env = env_file
	M.command()
